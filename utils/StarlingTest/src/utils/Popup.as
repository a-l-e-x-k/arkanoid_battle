...
{
	private static var popups:Vector.<Popup> = new Vector.<Popup>();
	...
	protected function onAddedToStage(event:Event):void
	{
		...
		if (popups.length > 0 && !_forced) //there are other popups in queue, wait until they will be removed
			visible = false;
	...
	protected function onRemovedFromStage(event:Event):void
	{
		...
		if (popups.length > 0 && !_forced) //if there are any other popups in the queue -> show first one of them
	...
	{
		var shader:Shape = Misc.createRectangle(Config.APP_WIDTH, Config.APP_HEIGHT);
		shader.alpha = _shaderAlpha;
		shader.addEventListener(MouseEvent.CLICK, die);
		super.addChild(shader);
	}
	...
	protected function die(event:MouseEvent = null):void
	{
		clearListeners();
